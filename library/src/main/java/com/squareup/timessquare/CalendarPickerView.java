...
public class CalendarPickerView extends ListView {
    ...
    private void validateDate(Date date) {
        if (date == null) {
            String str = "Selected date must be non-null.";
...
//          clearOldSelections();
                    ...
                    current.setTime(selectedCals.get(0).getTime());
                    current.add(DATE, 1);
                    if (newlySelectedCal.getTime().before(current.getTime())) {
                        return false;
                    }
...
//          clearOldSelections();
        ...
        if (date != null) {
            ...
            if (selectedCals.size() >= 2) {
                if (newlySelectedCal.getTime().before(selectedCals.get(1).getTime())) {
                    Calendar calendar = new GregorianCalendar();
                    calendar.setTime(selectedCals.get(1).getTime());
                    calendar.add(DATE, 1);
                    Date nextDate = calendar.getTime();
                    for (MonthCellDescriptor selectedCell : selectedCells) {
                        if (selectedCell.getDate().after(newlySelectedCal.getTime()) && selectedCell.getDate().before(nextDate)) {
                            selectedCell.setSelected(false);
                        }
                    }
                    selectedCells.get(1).setRangeState(RangeState.NONE);
                }
                selectedCals.set(1, newlySelectedCal);
                selectedCells.set(1, cell);
            } else {
